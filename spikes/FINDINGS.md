...
## Phase 3 follow-up spike — parallel tool calls (spike-07)

Flagged as untested in the original Phase 3 spike. `spike-07-parallel-tool-calls.ts` asked the model to call two independent tools (`get_weather` + `get_time`) in one turn and checked whether both got captured.

**Bug found:** the original approach — interrupt the query as soon as the FIRST `canUseTool` call fires — cut the query off before a SECOND, parallel `tool_use` block's own `canUseTool` invocation even happened. A live run confirmed the model proposed 2 `tool_use` blocks (visible in the assistant message and in the SDK's own `permission_denials`), but only 1 was captured; the interrupted query's `permission_denials` array also only ever recorded the first.

**Fix:** debounce the interrupt instead of firing on the first capture — each new `canUseTool` invocation resets a short (50ms) timer, so `interrupt()` only actually fires once no NEW capture has arrived for that window, giving same-turn sibling calls time to land first. Verified reliable: 3/3 live runs correctly captured both calls after the fix (0/1 correctly captured before it, confirming the bug was real and reproducible). Applied to `providers/claude.ts`'s `buildToolInterception` (`PARALLEL_CALL_DEBOUNCE_MS`), live-verified again against the real gateway on both routes after the change (2/3 genuine parallel-call runs correctly captured on the OpenAI route, 1/1 on Anthropic — the model doesn't always choose to call both tools in the same turn, that's normal model behavior, not a gateway bug; the fix's job is only to correctly capture whatever the model actually proposes in one turn). Adds a fixed 50ms latency to every tool-call-stop response, negligible next to multi-second Claude response times.
